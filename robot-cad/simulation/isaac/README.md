...
| `simulation/isaac/rl/stairs/_quadruped_stairs_env.py` | Extends walking with exact DOF order, dynamic swing/support residual scales, pre-play force tracking, five-phase placement, mass-weighted whole-robot COM regulation, per-leg slip and reachable-reference metrics, optional foot-friction sensitivity, selectable analytic/VL53L5CX perception, curriculum, and episode gates | stair world and task config | actions, observations, rewards, contact/sensor and episode state |
