...
| `simulation/isaac/rl/stairs/quadruped_stairs_v9_front_pair_placement.yaml` | Extends the fixed-tread reference to an experimental front-left/front-right sequence with mixed-height support; current diagnostic still fails body transfer | edited by stair runners | no generated data |
...
| `simulation/isaac/rl/stairs/_stair_rl_contract.py` | Defines selectable analytic or hardware-shaped terrain input, 57/60/68/81/84-value observation contracts, placement phases/contact gates, progress gates, strict stair/foot goals, physical-height reward terms, and failure reasons | walking/terrain/placement observation and stair YAML values | observations and scalar contract results |
| `simulation/isaac/rl/stairs/_quadruped_stairs_env.py` | Extends walking with exact DOF order, optional residual/reference actions, pre-play force tracking, five-phase placement, support load/slip/margin metrics, ordered-leg state, selectable analytic/VL53L5CX perception, curriculum, and episode metrics | stair world and task config | actions, observations, rewards, contact/sensor and episode state |
...
| `simulation/isaac/rl/stairs/train_stairs_ppo.py` | Trains, transfers, or resumes the separate stairs PPO policy, including 48-value shared-prefix transfer from unsupported balance and placement-aware observation sizing, with curriculum, checkpoint manifests, live metrics, and no-progress abort reports | stair YAML/world and optional model | model ZIPs, manifests, TensorBoard, monitor CSV, progress watchdog, training JSON |
...
| `simulation/isaac/rl/stairs/evaluate_stairs_ppo.py` | Runs deterministic stair episodes at a pinned curriculum level, verifies the model contract, and uses a close external camera for placement tasks | stair YAML/world, model, manifest | evaluation JSON and optional PNG |
| `simulation/isaac/rl/stairs/record_stairs_ppo.py` | Records deterministic or stochastic stair episodes, optionally replaying seeded precursor episodes and using a close placement view so policy RNG, PhysX history, and visual evidence remain exact | stair YAML/world, model, manifest | H.264 MP4, thumbnail PNG, optional trajectory, recording JSON |
