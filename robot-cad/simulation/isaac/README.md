...
| `simulation/isaac/rl/foot_lift/_quadruped_foot_lift_env.py` | Applies the raise-forward IK reference, optional torso pose hold, bounded PPO residuals, foot-tip measurements, and episode metrics | manual world and foot-lift config | actions, observations, rewards, and supported-lift metrics |
| `simulation/isaac/rl/foot_lift/train_foot_lift_ppo.py` | Runs smoke/full PPO training, optional flat-policy transfer, checkpointing, and schema-2 model packaging | foot-lift YAML/world and optional flat initializer | model ZIP, manifest, monitor/TensorBoard data, and training report |
