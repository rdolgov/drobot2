param(
    ...
    [string]$OutputDir = 'simulation/isaac/output/rl/ppo-stairs-v39-rear-right-landing-512-seed851',
    [int]$Seed = 851
)
...
try {
    & $IsaacPython simulation/isaac/rl/stairs/train_stairs_ppo.py `
        --config simulation/isaac/rl/stairs/quadruped_stairs_v39_rear_right_landing.yaml `
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "V39 rear-right landing PPO training failed with exit code $LASTEXITCODE"
    }
}
finally {
    Pop-Location
