param(
    ...
    [string]$OutputDir = 'simulation/isaac/output/rl/ppo-stairs-v35-rear-right-190mm-small'
)
...
try {
    & $IsaacPython simulation/isaac/rl/stairs/train_stairs_ppo.py `
        --config simulation/isaac/rl/stairs/quadruped_stairs_v35_full_first_tread.yaml `
        ...
        --initialize-from-stairs simulation/isaac/models/ppo-stairs-v35-rear-right-190mm-lift-small/initial_compact_lift_512_steps.zip

    if ($LASTEXITCODE -ne 0) {
        throw "V35 rear-right PPO training failed with exit code $LASTEXITCODE"
    }
}
finally {
    Pop-Location
