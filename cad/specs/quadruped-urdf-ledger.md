...
- Editable source of truth:
  `drobot_cad/urdf/quadruped_robot.py::gen_urdf()`.
...
- Geometry authority, in descending order:
  1. `drobot_cad/assembly/quadruped_robot.py`;
  2. `drobot_cad/assembly/robot_leg.py`;
...
The separately mounted physical one-leg testbed was exercised on 2026-07-28
with software ranges of `-45 to +45 deg`, `-90 to +90 deg`, and
`-120 to +120 deg`, respectively, and the operator reported free motion. That
is useful evidence about the isolated leg and joint-frame signs, but it does
not widen these whole-robot limits: body interference, other-leg interference,
cable routing, ground contact, and loaded operation are absent from the wall
fixture. The matching Isaac fixture is generated by
`drobot_cad/urdf/one_leg_wall_testbed.py`.
...
- `simulation/exports/isaac/quadruped_robot_fixed.usdc`: self-contained fixed-base
  commissioning articulation;
- `simulation/exports/isaac/quadruped_robot_floating.usdc`: self-contained floating-base
  dynamics articulation with guarded inter-leg collision;
- `simulation/exports/isaac/quadruped_robot_manual_world.usda`: portable world referencing
