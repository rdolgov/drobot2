...
- Keep the topic documentation indexed in `docs/README.md` current. Every CAD,
  mechanical-specification, or CAD-derived URDF change must update its owning
  Markdown file in the same change. Non-CAD work belongs in the sibling
  `simulation/`, `hardware/`, or `electrical/` package. Record what changed,
