...
```text
cad/drobot_cad/urdf/quadruped_robot.py
                 |
                 v
cad/exports/urdf/quadruped_robot.urdf
                 |
                 v
...
fixed robot USDC             floating robot USDC
          ...
          |                         |
          |                         v
          |       simulation/exports/isaac/quadruped_robot_manual_world.usda
          |                         |
          v                         v
...
Generate, import, and sweep the explicit artifacts from the repository root:

```powershell
$projectPython = '.\cad\.venv\Scripts\python.exe'
...
& $projectPython $urdfTool `
  cad\drobot_cad\urdf\one_leg_wall_testbed.py `
  -o cad\exports\urdf\one_leg_wall_testbed.urdf

& $isaacPython simulation\isaac\import_one_leg_wall.py `
  --urdf cad\exports\urdf\one_leg_wall_testbed.urdf `
  --output simulation\exports\isaac\one_leg_wall_testbed.usdc `
  --report simulation\isaac\output\one-leg-wall\import_report.json

& $isaacPython simulation\isaac\run_one_leg_wall.py `
  --gravity both `
  --screenshot simulation\reviews\isaac-one-leg-wall-range.png
```
...
The reviewed render is `simulation/reviews/isaac-one-leg-wall-range.png`.
...
& $isaacPython simulation\isaac\open_articulation.py `
  --world simulation\exports\isaac\quadruped_robot_manual_world.usda `
  --report simulation\isaac\output\manual-open.json
```

The committed `quadruped_robot_manual_world.usda` references
`quadruped_robot_floating.usdc` in the same `simulation/exports/isaac` directory. Git LFS
...
| --- | --- | --- | --- |
| `simulation/scripts/setup_isaac_rl.ps1` | Installs and verifies the pinned PPO dependencies in Isaac's Python | `simulation/isaac/rl/requirements.txt` | Isaac Python environment |
...
| `Drobot-Pure-Stairs-Forward-Neutral-Foot-Lift7p5-PersistentBias-CEM-Robust-Hip-Direct` | Rotates the grounded neutral robot to face the stairs and restores the VL53L5CX-style depth rays to the forward body mount | same 70-value deployable actor contract, reward, limits, and checkpoint architecture | current hands-on preview and forward-facing PPO checkpoints |
...
| `simulation/isaac/rl/parallel_stairs/run_stair_rl_workflow.ps1` | Runs the current 7.5 cm checkpoint from a grounded forward-facing stance as one visible robot, a bounded five-robot GUI fine-tune, or a long 128-robot headless continuation | latest forward checkpoint, prior sideways-neutral checkpoint, or packaged checkpoint | visual inspection and resumable forward-facing RSL-RL checkpoints |

### Hands-on current-model workflow

Run these commands from the repository root in order. The first command opens one
...
grounded 0.3305 m stance facing the stairs, keep the depth rays on the forward
body mount, the 0.8825985 N m effort cap, and 70-value deployable
IMU/joint/load/depth actor input. `-Checkpoint <path>` explicitly selects a
different initializer. Training outputs remain under
`logs/rsl_rl/drobot_pure_stairs_forward_neutral_foot_lift7p5_persistent_bias_cem_robust_hip_180x250_direct/`.
...
- Curated handoff assets in `simulation/exports/isaac/` are committed through Git LFS.
...
- Intentional review PNGs belong in `simulation/reviews/` and are committed through Git
...
```powershell
$isaacPython = 'C:\isaacsim\python.bat'
$urdf = (Resolve-Path 'cad\exports\urdf\quadruped_robot.urdf').Path
...
replacing the curated files in `simulation/exports/isaac/`. Promotion is deliberately not
...
& C:\isaacsim\python.bat simulation\isaac\validate_camera.py `
  --usd simulation\exports\isaac\quadruped_robot_fixed.usdc `
...
- `simulation/exports/isaac/quadruped_robot_floating.usdc`
- `simulation/exports/isaac/quadruped_robot_manual_world.usda`
