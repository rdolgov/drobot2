"""Search a positive-margin COM target for the front-left to rear-right handoff.

The verified front-foot policies are replayed once. Each candidate then restores
the same physical transfer-start snapshot, shifts only the retained COM target,
and runs the analytic transfer with zero residual action. This keeps target
geometry separate from PPO and makes the subsequent transfer training start
from an evidence-backed support target.
"""
...
parser.add_argument(
    "--config",
    default=str(SCRIPT_DIR / "quadruped_stairs_v37_joint_clearance_support.yaml"),
)
...
parser.add_argument(
    ...
    default=(
        ...
        "rear-transfer-com-search-v38-seed832.json"
    ),
)
parser.add_argument(
    "--front-right-model",
...
try:
    ...
    print("DROBOT_REAR_TRANSFER_SEARCH_PHASE=precursor_replay", flush=True)
    ...
    if phase_env.phase_snapshot is None or not raw_env.placement_transfer_active:
        raise RuntimeError("rear-right transfer-start snapshot was not captured")
    ...
    for forward_delta_m in forward_deltas_m:
        for lateral_delta_m in lateral_deltas_m:
            ...
            if completion_metrics.get("transition") != "front_left->rear_right":
                completion_metrics = {}
            ...
            print(
                "DROBOT_REAR_TRANSFER_CANDIDATE="
                + json.dumps(
                    {
                        "id": result["id"],
                        "completed": completed,
    ...
    with report_path.open("w", encoding="utf-8") as stream:
        ...
        stream.write("\n")
    print("DROBOT_REAR_TRANSFER_SEARCH=" + json.dumps(report, sort_keys=True))
