param(
    ...
    [string]$OutputDir = 'simulation/isaac/output/rl/ppo-stairs-v42-com-margin-rear-right-landing-1024-seed865',
    [int]$Seed = 865
)
...
try {
    & $IsaacPython simulation/isaac/rl/stairs/train_stairs_ppo.py `
        --config simulation/isaac/rl/stairs/quadruped_stairs_v42_com_margin_rear_right_landing.yaml `
        ...
        --initialize-from-stairs simulation/isaac/models/ppo-stairs-v38-rear-right-190mm-small/drobot_stairs_ppo_final.zip `
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "V42 COM-margin landing PPO failed with exit code $LASTEXITCODE"
    }
}
finally {
    Pop-Location
