...
MODEL_METADATA="${REPO_ROOT}/onboard/models/parallel-walking-v23-higher-speed-straight-residual-crawl/model_1500.json"
...
if head -n 1 "${MODEL_PATH}" | grep -q '^version https://git-lfs'; then
  ...
  git -C "${REPO_ROOT}" lfs pull \
    --include="onboard/models/parallel-walking-v23-higher-speed-straight-residual-crawl/model_1500.onnx"
fi
