...
MODEL_METADATA="${REPO_ROOT}/onboard/models/parallel-walking-v30-symmetry-gated-robust-straight-crawl/model_5000.json"
...
if head -n 1 "${MODEL_PATH}" | grep -q '^version https://git-lfs'; then
  ...
  git -C "${REPO_ROOT}" lfs pull \
    --include="onboard/models/parallel-walking-v30-symmetry-gated-robust-straight-crawl/model_5000.onnx"
fi
